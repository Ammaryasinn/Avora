      ...
      <div className="mt-8 premium-panel overflow-hidden rounded-3xl">
        {leads.length ? leads.map((lead) => (
          <Link key={lead.id} href={`/dashboard/${organizationSlug}/leads/${lead.id}`} className="grid gap-4 border-b border-border p-5 transition last:border-b-0 hover:bg-surface-muted/45 md:grid-cols-[1.2fr_0.8fr_0.8fr_auto] md:items-center">
            <div><p className="font-semibold">{lead.contact.displayName ?? lead.contact.phoneE164 ?? lead.contact.waId}</p><p className="mt-1 text-xs text-text-muted">{lead.source.replaceAll("_", " ")} · {lead._count.conversations} conversation(s)</p></div>
            ...
            <div><p className="text-xs uppercase tracking-wider text-text-muted">Consent</p><p className="mt-1 text-sm">{lead.followUpState?.consentStatus ?? "UNKNOWN"}</p></div>
